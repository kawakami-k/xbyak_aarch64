...
public:
    ...
    void gen_exp_body(const ZRegS &t0) {
        ...
        fmin(t0, p_all, exp_ln_flt_max_f);
        fmax(t0, p_all, exp_ln_flt_min_f);
        fmul(t0, t0, exp_log2ef);
        movprfx(t1, p_all, t0);
        frintm(t1, p_all, t0);
        fcvtzs(t2, p_all, t1);
        fsub(t1, t0, t1);
        ...
        fexpa(t1, t1);
        fscale(t1, p_all, t2);
        and_(ZRegD(t2.getIdx()), ZRegD(t0.getIdx()),
                ZRegD(exp_not_mask17.getIdx()));
        fsub(t2, t0, t2);
        movprfx(t0, p_all, exp_coeff2);
        fmad(t0, p_all, t2, exp_coeff1);
        fmad(t0, p_all, t2, exp_one);
        fmul(t0, t1, t0);
    }
