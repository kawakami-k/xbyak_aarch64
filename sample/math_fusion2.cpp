...
public:
    ...
    void gen_exp_body(const ZRegS &t0) {
        const ZRegS t1 = z_aux[7].s;
        const ZRegS t2 = z_aux[8].s;
        ...
        loop_mn(0, 16, mn_fmin, p_all, exp_ln_flt_max_f);
        loop_mn(0, 16, mn_fmax, p_all, exp_ln_flt_min_f);
        for (size_t i = 0; i < UNROLL; i++)
            fmul(ZRegS(i), ZRegS(i), exp_log2ef);

        for (size_t i = 0; i < UNROLL; i++) {
            movprfx(t1, p_all, ZRegS(i));
            frintm(t1, p_all, ZRegS(i));
            fcvtzs(t2, p_all, t1);
            fsub(t1, ZRegS(i), t1);
            fadd(ZRegS(i), t1, exp_one);
            lsr(t1, ZRegS(i), 17);
            fexpa(t1, t1);
            fscale(t1, p_all, t2);
            and_(ZRegD(t2.getIdx()), ZRegD(ZRegS(i).getIdx()),
                    ZRegD(exp_not_mask17.getIdx()));
            fsub(t2, ZRegS(i), t2);
            movprfx(ZRegS(i), p_all, exp_coeff2);
            fmad(ZRegS(i), p_all, t2, exp_coeff1);
            fmad(ZRegS(i), p_all, t2, exp_one);
            fmul(ZRegS(i), t1, ZRegS(i));
        }
    }
