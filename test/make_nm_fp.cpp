...
const uint64_t IMM5BIT_N =
    1ULL << flagBit++; /** Test vector is {0, 1, 2, .., 32 } */
const uint64_t IMM6BIT_N =
    1ULL << flagBit++; /** Test vector is {0, 1, 2, .., 64 } */
...
class Test {
  ...
  std::vector<std::vector<std::string> *> tv_VectorsAs = {
      &tv_WREG,         &tv_XREG,           &tv_WSP,
      &tv_XSP,          &tv_IMM4BIT,        &tv_IMM5BIT,
      &tv_IMM6BIT,      &tv_IMM8BIT,        &tv_IMM12BIT,
      &tv_IMM13BIT,     &tv_IMM16BIT,       &tv_IMM5BIT_N,
      &tv_IMM6BIT_N,    &tv_FLOAT8BIT,      &tv_COND,
      &tv_COND_WO_AL,   &tv_NZCV,           &tv_BREG,
      &tv_HREG,         &tv_SREG,           &tv_DREG,
      ...
      &tv_EXT_AMOUNT32, &tv_EXT_AMOUNT64,   &tv_VREG_ELEM};
...
public:
  ...
  */
  ...
  /**** Floating-point move register without conversion FMOV (register) on page
   * C7-1533 */
  PUT3(DataProcFp_FloatingPointMoveRegister0, NM("fmov"), OPS(HREG, HREG),
       OPS(SREG, SREG), OPS(DREG, DREG));
  /**** Floating-point move to or from general-purpose register without
   * conversion FMOV (general) on page C7-1535 */
  PUT8(DataProcFp_FloatingPointMoveRegister1, NM("fmov"), OPS(WREG, HREG),
       OPS(XREG, HREG), OPS(HREG, WREG), OPS(SREG, WREG), OPS(WREG, SREG),
       OPS(HREG, XREG), OPS(DREG, XREG), OPS(VREG_ELEM, XREG));
  PUT2(DataProcFp_FloatingPointMoveRegister2, NM("fmov"), OPS(XREG, DREG),
       OPS(XREG, VREG_ELEM));
  ...
  /**** FMOV Floating-point move immediate FMOV (scalar, immediate) on page
   * C7-1538 */
  PUT3(DataProcFp_FloatingPointMoveImmediate0, NM("fmov"), OPS(HREG, FLOAT8BIT),
       OPS(SREG, FLOAT8BIT), OPS(DREG, FLOAT8BIT));
  ...
  PUT6(DataProcFp_FloatingPointConversion0, NM("fcvt"), OPS(SREG, HREG),
       OPS(DREG, HREG), OPS(HREG, SREG), OPS(DREG, SREG), OPS(HREG, DREG),
       OPS(SREG, DREG));
  /**** FCVTAS Floating-point scalar convert to signed integer, rounding to
     nearest with ties to away (scalar form) FCVTAS (scalar) on page C7-1401 */
  ...
  /**** FCVTMU Floating-point scalar convert to unsigned integer, rounding
     toward minus infinity (scalar form) FCVTMU (scalar) on page C7-1418 */
  /**** FCVTNS Floating-point scalar convert to signed integer, rounding to
     nearest with ties to even (scalar form) FCVTNS (scalar) on page C7-1425 */
  ...
  /**** FCVTPU Floating-point scalar convert to unsigned integer, rounding
     toward positive infinity (scalar form) FCVTPU (scalar) on page C7-1440 */
  /**** FCVTZS Floating-point scalar convert to signed integer, rounding toward
        zero (scalar form)
        FCVTZS (scalar, integer) on page C7-1452 */
  /**** FCVTZU Floating-point scalar convert to unsigned integer, rounding
     toward zero (scalar form) FCVTZU (scalar, integer) on page C7-1462 */
  PUT6(DataProcFp_FloatingPointConversion1,
       NM("fcvtas", "fcvtau", "fcvtms", "fcvtmu", "fcvtns", "fcvtnu", "fcvtps",
          "fcvtpu", "fcvtzs", "fcvtzu"),
  ...
  PUT6(DataProcFp_FloatingPointConversion2, NM("fcvtzs", "fcvtzu"),
  ...
  /**** FJCVTZS Floating-point Javascript convert to signed fixed-point,
     rounding toward zero FJCVTZS on page C7-1468 */
  PUT1(DataProcFp_FloatingPointConversion3, NM("fjcvtzs"), OPS(WREG, DREG));
  ...
  /**** UCVTF Unsigned integer scalar convert to floating-point, using the
     current rounding mode (scalar form)
        UCVTF (scalar, integer) on page C7-2032 */
  PUT6(DataProcFp_FloatingPointConversion4, NM("scvtf", "ucvtf"),
       OPS(HREG, WREG), OPS(SREG, WREG), OPS(DREG, WREG), OPS(HREG, XREG),
  ...
  PUT6(DataProcFp_FloatingPointConversion5, NM("scvtf", "ucvtf"),
  ...
  PUT3(DataProcFp_FloatingPointMultiplyAdd0,
       NM("fmadd", "fmsub", "fnmadd", "fnmsub"), OPS(HREG, HREG, HREG, HREG),
       OPS(SREG, SREG, SREG, SREG), OPS(DREG, DREG, DREG, DREG));
  ...
  PUT3(DataProcFp_FloatingPointArithmeticOneSource0,
       NM("fabs", "fneg", "fsqrt"), OPS(HREG, HREG), OPS(SREG, SREG),
       OPS(DREG, DREG));
  ...
  PUT3(DataProcFp_FloatingPointArithmeticTwoSources0,
       NM("fadd", "fdiv", "fmul", "fnmul", "fsub"), OPS(HREG, HREG, HREG),
  ...
  PUT3(DataProcFp_FloatingPointMinimumAndMaximum0,
       NM("fmax", "fmaxnm", "fmin", "fminnm"), OPS(HREG, HREG, HREG),
       OPS(SREG, SREG, SREG), OPS(DREG, DREG, DREG));
  ...
  /**** C7.2.59 FCMP */
  /**** C7.2.60 FCMPE */
  PUT6(DataProcFp_FloatingPointComparison0, NM("fcmp", "fcmpe"),
       OPS(HREG, HREG), OPS(HREG, SPECIFIC64), OPS(SREG, SREG),
       OPS(SREG, SPECIFIC64), OPS(DREG, DREG), OPS(DREG, SPECIFIC64));
  /**** C7.2.47 FCCMP */
  /**** C7.2.48 FCCMPE */
  PUT3(DataProcFp_FloatingPointComparison1, NM("fccmp", "fccmpe"),
       OPS(HREG, HREG, NZCV, COND), OPS(SREG, SREG, NZCV, COND),
       OPS(DREG, DREG, NZCV, COND));
  ...
  PUT3(DataProcFp_FloatingPointConditionalSelect0, NM("fcsel"),
       OPS(HREG, HREG, HREG, COND), OPS(SREG, SREG, SREG, COND),
       OPS(DREG, DREG, DREG, COND));
