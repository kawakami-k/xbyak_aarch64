    ...
    for (int i : regIdx) {
      ...
      for (size_t j = 0; j < type.size(); j++) {
        ...
        reg2D[j]->push_back("{ v" + tmp0 + "." + type[j] + ", v" + tmp1 + "." +
                            type[j] + " }");
        jreg2D[j]->push_back("{ v" + tmp0 + "." + jtype[j] + " - v" + tmp1 +
                             "." + jtype[j] + " }");

        reg3D[j]->push_back("{ v" + tmp0 + "." + type[j] + ", v" + tmp1 + "." +
                            type[j] + ", v" + tmp2 + "." + type[j] + " }");
        jreg3D[j]->push_back("{ v" + tmp0 + "." + jtype[j] + " - v" + tmp2 +
                             "." + jtype[j] + " }");

        reg4D[j]->push_back("{ v" + tmp0 + "." + type[j] + ", v" + tmp1 + "." +
                            type[j] + ", v" + tmp2 + "." + type[j] + ", v" +
                            tmp3 + "." + type[j] + " }");
        jreg4D[j]->push_back("{ v" + tmp0 + "." + jtype[j] + " - v" + tmp3 +
                             "." + jtype[j] + " }");
      }
    }
    ...
    if (num == 1) {
      ...
      for (int i : reg_idx) {
        ...
        tv.push_back("{ v" + std::to_string(i) + "." + type + ", v" +
                     ...
                     "]");
        jtv.push_back("( v" + std::to_string(i) + "." + jtype + " - v" +
                      std::to_string((i + 1) % 32) + "." + jtype + " )[" + tmp +
                      "]");
      }
      /*** Element index rotation */
      for (int i : elem_idx) {
        ...
        tv.push_back("{ v" + std::to_string(reg_idx[0]) + "." + type + ", v" +
                     ...
                     " }[" + tmp + "]");
        jtv.push_back("( v" + std::to_string(reg_idx[0]) + "." + jtype +
                      " - v" + std::to_string((reg_idx[0] + 1) % 32) + "." +
                      jtype + " )[" + tmp + "]");
      }
    } else if (num == 3) {
      /** Three register */
      ...
      for (int i : reg_idx) {
        ...
        tv.push_back("{ v" + std::to_string(i) + "." + type + ", v" +
                     ...
                     "]");
        jtv.push_back("( v" + std::to_string(i) + "." + jtype + " - v" +
                      std::to_string((i + 2) % 32) + "." + jtype + " )[" + tmp +
                      "]");
      }
      /*** Element index rotation */
      for (int i : elem_idx) {
      ...
      for (int i : reg_idx) {
        ...
        tv.push_back("{ v" + std::to_string(i) + "." + type + ", v" +
                     std::to_string((i + 1) % 32) + "." + type + ", v" +
                     std::to_string((i + 2) % 32) + "." + type + ", v" +
                     std::to_string((i + 3) % 32) + "." + type + " }[" + tmp +
                     "]");
        jtv.push_back("( v" + std::to_string(i) + "." + jtype + " - v" +
                      std::to_string((i + 3) % 32) + "." + jtype + " )[" + tmp +
                      "]");
      }
      /*** Element index rotation */
      for (int i : elem_idx) {
        ...
        tv.push_back("{ v" + std::to_string(reg_idx[0]) + "." + type + ", v" +
                     ...
                     "." + type + " }[" + tmp + "]");
        jtv.push_back("( v" + std::to_string(reg_idx[0]) + "." + jtype +
                      " - v" + std::to_string((reg_idx[0] + 3) % 32) + "." +
                      jtype + " )[" + tmp + "]");
      }
    } else {
...
public:
  ...
  */
  ...
  /*** LDR Load scalar SIMD&FP register (register offset) LDR (register,
   * SIMD&FP) on page C7-1673 */
  /*** STR Store scalar SIMD&FP register (register offset) STR (register,
   * SIMD&FP) on page C7-1985 */
  PUT7(Load_Store_scalar_SIMD_and_floating_point0, NM("ldr", "str"),
  ...
  PUT7(Load_Store_scalar_SIMD_and_floating_point1, NM("ldr"),
  ...
  PUT7(Load_Store_scalar_SIMD_and_floating_point2, NM("ldr"),
  ...
  PUT7(Load_Store_scalar_SIMD_and_floating_point3, NM("ldr"),
  ...
  PUT7(Load_Store_scalar_SIMD_and_floating_point4, NM("ldr"),
   ...
   * on page C7-1981 */
  PUT5(Load_Store_scalar_SIMD_and_floating_point5, NM("ldr", "str"),
       /**** Post-index */
       OPS(BREG, SPECIFIC3), OPS(HREG, SPECIFIC3), OPS(SREG, SPECIFIC3),
       OPS(DREG, SPECIFIC3), OPS(QREG, SPECIFIC3));
  PUT5(Load_Store_scalar_SIMD_and_floating_point6, NM("ldr"),
       /**** Pre-index */
  ...
  PUT10(Load_Store_scalar_SIMD_and_floating_point7, NM("ldr"),
        /*** Unsigned offset */
        OPS(BREG, PTR_O, XNSP, PTR_C), OPS(BREG, PTR_O, XNSP, IMM12BIT, PTR_C),
        OPS(HREG, PTR_O, XNSP, PTR_C),
        OPS(HREG, PTR_O, XNSP, IMM12BIT_MUL2, PTR_C),
        OPS(SREG, PTR_O, XNSP, PTR_C),
        OPS(SREG, PTR_O, XNSP, IMM12BIT_MUL4, PTR_C),
        OPS(DREG, PTR_O, XNSP, PTR_C),
        OPS(DREG, PTR_O, XNSP, IMM12BIT_MUL8, PTR_C),
        OPS(QREG, PTR_O, XNSP, PTR_C),
        OPS(QREG, PTR_O, XNSP, IMM12BIT_MUL16, PTR_C));
  /*** Load scalar SIMD&FP register (PC-relative literal) LDR (literal, SIMD&FP)
   * on page C7-1671 */
  PUT3(Load_Store_scalar_SIMD_and_floating_point8, NM("ldr"),
       OPS(SREG, IMM19BIT_MUL4), OPS(DREG, IMM19BIT_MUL4),
       OPS(QREG, IMM19BIT_MUL4));
  /*** LDUR Load scalar SIMD&FP register (unscaled offset) LDUR (SIMD&FP) on
   * page C7-1676 */
  /*** STUR Store scalar SIMD&FP register (unscaled offset) STUR (SIMD&FP) on
   * page C7-1988 */
  PUT10(
      Load_Store_scalar_SIMD_and_floating_point9, NM("ldur", "stur"),
   ...
   */
  PUT6(Load_Store_scalar_SIMD_and_floating_point10, NM("ldp"),
       /**** Post-index */
       /***** if t == t2 then result is implementation dependent. */
       OPS(SREG, SREG2, SPECIFIC0), OPS(DREG, DREG2, SPECIFIC1),
       OPS(QREG, QREG2, SPECIFIC2), OPS(SREG2, SREG, SPECIFIC0),
       OPS(DREG2, DREG, SPECIFIC1), OPS(QREG2, QREG, SPECIFIC2));
  PUT6(Load_Store_scalar_SIMD_and_floating_point11, NM("ldp"),
       /**** Pre-index */
       /***** if t == t2 then result is implementation dependent. */
  ...
  PUT6(Load_Store_scalar_SIMD_and_floating_point12, NM("ldp"),
       /**** Singed offset */
       /***** if t == t2 then result is implementation dependent. */
  ...
  PUT6(Load_Store_scalar_SIMD_and_floating_point13, NM("ldp"),
       /**** Singed offset */
       /***** if t == t2 then result is implementation dependent. */
   ...
   */
  PUT6(Load_Store_scalar_SIMD_and_floating_point14, NM("stp"),
       /**** Post-index */
       OPS(SREG, SREG, SPECIFIC0), OPS(DREG, DREG, SPECIFIC1),
  ...
  PUT6(Load_Store_scalar_SIMD_and_floating_point15, NM("stp"),
       /**** Singed offset */
       OPS(SREG, SREG, PTR_O, XNSP, PTR_C), OPS(DREG, DREG, PTR_O, XNSP, PTR_C),
   ...
   */
  PUT6(Load_Store_scalar_SIMD_and_floating_point16, NM("ldnp"),
       /**** if t == t2 then result is implementation dependent. */
       OPS(SREG, SREG2, PTR_O, XNSP, PTR_C),
       OPS(SREG, SREG2, PTR_O, XNSP, IMM7BIT_PM_MUL4, PTR_C),
       OPS(DREG, DREG2, PTR_O, XNSP, PTR_C),
       ...
       OPS(QREG, QREG2, PTR_O, XNSP, PTR_C),
       OPS(QREG, QREG2, PTR_O, XNSP, IMM7BIT_PM_MUL16, PTR_C));
  PUT6(Load_Store_scalar_SIMD_and_floating_point17, NM("ldnp"),
       /**** if t == t2 then result is implementation dependent. */
       OPS(SREG2, SREG, PTR_O, XNSP, PTR_C),
       OPS(SREG2, SREG, PTR_O, XNSP, IMM7BIT_PM_MUL4, PTR_C),
       OPS(DREG2, DREG, PTR_O, XNSP, PTR_C),
       ...
       OPS(QREG2, QREG, PTR_O, XNSP, PTR_C),
       OPS(QREG2, QREG, PTR_O, XNSP, IMM7BIT_PM_MUL16, PTR_C));
  /*** STNP Store pair of scalar SIMD&FP registers STNP (SIMD&FP) on page
   * C7-1976 */
  PUT6(Load_Store_scalar_SIMD_and_floating_point18, NM("stnp"),
  ...
  /*** Load single 1-element structure to one lane of one register LD1 (single
   * structure) on page C7-1625 */
  /*** Store single 1-element structure from one lane of one register ST1
   * (single structure) on page C7-1951 */
  PUT8(Load_Store_Vector0, NM("ld1", "st1"),
       /**** No offset */
       ...
       /**** Post-index */
       OPS(VREGB_1D_ELEM, SPECIFIC0), OPS(VREGH_1D_ELEM, SPECIFIC1),
       OPS(VREGS_1D_ELEM, SPECIFIC2), OPS(VREGD_1D_ELEM, SPECIFIC3));
  /*** Load multiple 1-element structures to one register or to two, three, or
   * four consecutive registers LD1 (multiple structures) on page C7-1621 */
  /*** Store multiple 1-element structures from one register, or from two,
   * three, or four consecutive registers ST1 (multiple structures) on page
   * C7-1947 */
  PUT16(
      Load_Store_Vector1, NM("ld1", "st1"),
      /**** No offset */
      OPS(VREG16B_1D, PTR_O, XNSP2, PTR_C), OPS(VREG8B_1D, PTR_O, XNSP2, PTR_C),
      OPS(VREG8H_1D, PTR_O, XNSP2, PTR_C), OPS(VREG4H_1D, PTR_O, XNSP2, PTR_C),
      OPS(VREG4S_1D, PTR_O, XNSP2, PTR_C), OPS(VREG2S_1D, PTR_O, XNSP2, PTR_C),
      OPS(VREG2D_1D, PTR_O, XNSP2, PTR_C), OPS(VREG1D_1D, PTR_O, XNSP2, PTR_C),
      OPS(VREG16B_2D, PTR_O, XNSP2, PTR_C), OPS(VREG8B_2D, PTR_O, XNSP2, PTR_C),
      OPS(VREG8H_2D, PTR_O, XNSP2, PTR_C), OPS(VREG4H_2D, PTR_O, XNSP2, PTR_C),
      OPS(VREG4S_2D, PTR_O, XNSP2, PTR_C), OPS(VREG2S_2D, PTR_O, XNSP2, PTR_C),
      OPS(VREG2D_2D, PTR_O, XNSP2, PTR_C), OPS(VREG1D_2D, PTR_O, XNSP2, PTR_C));
  PUT16(
      Load_Store_Vector2, NM("ld1"),
      /**** No offset (Continued) */
      OPS(VREG16B_3D, PTR_O, XNSP2, PTR_C), OPS(VREG8B_3D, PTR_O, XNSP2, PTR_C),
      OPS(VREG8H_3D, PTR_O, XNSP2, PTR_C), OPS(VREG4H_3D, PTR_O, XNSP2, PTR_C),
      OPS(VREG4S_3D, PTR_O, XNSP2, PTR_C), OPS(VREG2S_3D, PTR_O, XNSP2, PTR_C),
      OPS(VREG2D_3D, PTR_O, XNSP2, PTR_C), OPS(VREG1D_3D, PTR_O, XNSP2, PTR_C),
      OPS(VREG16B_4D, PTR_O, XNSP2, PTR_C), OPS(VREG8B_4D, PTR_O, XNSP2, PTR_C),
      OPS(VREG8H_4D, PTR_O, XNSP2, PTR_C), OPS(VREG4H_4D, PTR_O, XNSP2, PTR_C),
      OPS(VREG4S_4D, PTR_O, XNSP2, PTR_C), OPS(VREG2S_4D, PTR_O, XNSP2, PTR_C),
      OPS(VREG2D_4D, PTR_O, XNSP2, PTR_C), OPS(VREG1D_4D, PTR_O, XNSP2, PTR_C));
  PUT16(Load_Store_Vector3, NM("ld1"),
        /**** Post-index */
        OPS(VREG16B_1D, SPECIFIC4), OPS(VREG8B_1D, SPECIFIC3),
        OPS(VREG8H_1D, SPECIFIC4), OPS(VREG4H_1D, SPECIFIC3),
        OPS(VREG4S_1D, SPECIFIC4), OPS(VREG2S_1D, SPECIFIC3),
        OPS(VREG2D_1D, SPECIFIC4), OPS(VREG1D_1D, SPECIFIC3),
        OPS(VREG16B_2D, SPECIFIC6), OPS(VREG8B_2D, SPECIFIC4),
        OPS(VREG8H_2D, SPECIFIC6), OPS(VREG4H_2D, SPECIFIC4),
        OPS(VREG4S_2D, SPECIFIC6), OPS(VREG2S_2D, SPECIFIC4),
        OPS(VREG2D_2D, SPECIFIC6), OPS(VREG1D_2D, SPECIFIC4));
  PUT16(Load_Store_Vector4, NM("ld1"),
        /**** Post-index (Continued) */
        OPS(VREG16B_3D, SPECIFIC7), OPS(VREG8B_3D, SPECIFIC5),
        OPS(VREG8H_3D, SPECIFIC7), OPS(VREG4H_3D, SPECIFIC5),
        OPS(VREG4S_3D, SPECIFIC7), OPS(VREG2S_3D, SPECIFIC5),
        OPS(VREG2D_3D, SPECIFIC7), OPS(VREG1D_3D, SPECIFIC5),
        OPS(VREG16B_4D, SPECIFIC8), OPS(VREG8B_4D, SPECIFIC6),
        OPS(VREG8H_4D, SPECIFIC8), OPS(VREG4H_4D, SPECIFIC6),
        OPS(VREG4S_4D, SPECIFIC8), OPS(VREG2S_4D, SPECIFIC6),
        OPS(VREG2D_4D, SPECIFIC8), OPS(VREG1D_4D, SPECIFIC6));
  /*** LD2 Load single 2-element structure to one lane of two consecutive
   * registers LD2 (single structure) on page C7-1635 */
  /*** Store single 2-element structure from one lane of two consecutive
   * registers ST2 (single structure) on page C7-1958 */
  PUT8(Load_Store_Vector5, NM("ld2", "st2"),
       // No offset
       ...
       // Post-index
       OPS(VREGB_2D_ELEM, SPECIFIC1), OPS(VREGH_2D_ELEM, SPECIFIC2),
       OPS(VREGS_2D_ELEM, SPECIFIC3), OPS(VREGD_2D_ELEM, SPECIFIC4));
  /*** Load multiple 2-element structures to two consecutive registers LD2
   * (multiple structures) on page C7-1632 */
  /*** Store multiple 2-element structures from two consecutive registers ST2
   * (multiple structures) on page C7-1955 */
  PUT7(Load_Store_Vector6, NM("ld2", "st2"),
       // No offset
       OPS(VREG8B_2D, PTR_O, XNSP2, PTR_C),
       OPS(VREG16B_2D, PTR_O, XNSP2, PTR_C),
       OPS(VREG4H_2D, PTR_O, XNSP2, PTR_C), OPS(VREG8H_2D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2S_2D, PTR_O, XNSP2, PTR_C), OPS(VREG4S_2D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2D_2D, PTR_O, XNSP2, PTR_C));
  PUT7(Load_Store_Vector7, NM("ld2"),
       // Post-index
       OPS(VREG8B_2D, SPECIFIC4), OPS(VREG16B_2D, SPECIFIC6),
       OPS(VREG4H_2D, SPECIFIC4), OPS(VREG8H_2D, SPECIFIC6),
       OPS(VREG2S_2D, SPECIFIC4), OPS(VREG4S_2D, SPECIFIC6),
       OPS(VREG2D_2D, SPECIFIC6));
  /*** Load single 3-element structure to one lane of three consecutive
   * registers LD3 (single structure) on page C7-1645 */
  /*** Store single 3-element structure from one lane of three consecutive
   * registers ST3 (single structure) on page C7-1965 */
  PUT8(Load_Store_Vector8, NM("ld3", "st3"),
       // No offset
       ...
       // Post-index
       OPS(VREGB_3D_ELEM, SPECIFIC9), OPS(VREGH_3D_ELEM, SPECIFIC10),
       OPS(VREGS_3D_ELEM, SPECIFIC11), OPS(VREGD_3D_ELEM, SPECIFIC5));
  /*** Load multiple 3-element structures to three consecutive registers LD3
   * (multiple structures) on page C7-1642 */
  /*** Store multiple 3-element structures from three consecutive registers ST3
   * (multiple structures) on page C7-1962 */
  PUT7(Load_Store_Vector9, NM("ld3", "st3"),
       // No offset
       OPS(VREG8B_3D, PTR_O, XNSP2, PTR_C),
       OPS(VREG16B_3D, PTR_O, XNSP2, PTR_C),
       OPS(VREG4H_3D, PTR_O, XNSP2, PTR_C), OPS(VREG8H_3D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2S_3D, PTR_O, XNSP2, PTR_C), OPS(VREG4S_3D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2D_3D, PTR_O, XNSP2, PTR_C));
  PUT7(Load_Store_Vector10, NM("ld3"),
       // Post-index
       OPS(VREG8B_3D, SPECIFIC5), OPS(VREG16B_3D, SPECIFIC7),
       OPS(VREG4H_3D, SPECIFIC5), OPS(VREG8H_3D, SPECIFIC7),
       OPS(VREG2S_3D, SPECIFIC5), OPS(VREG4S_3D, SPECIFIC7),
       OPS(VREG2D_3D, SPECIFIC7));
  /*** Load single 4-element structure to one lane of four consecutive registers
   * LD4 (single structure) on page C7-1655 */
  /*** Store single 4-element structure from one lane of four consecutive
   * registers ST4 (single structure) on page C7-1972 */
  PUT8(Load_Store_Vector11, NM("ld4", "st4"),
       // No offset
       ...
       // Post-index
       OPS(VREGB_4D_ELEM, SPECIFIC2), OPS(VREGH_4D_ELEM, SPECIFIC3),
       OPS(VREGS_4D_ELEM, SPECIFIC4), OPS(VREGD_4D_ELEM, SPECIFIC6));
  /*** Load multiple 4-element structures to four consecutive registers LD4
   * (multiple structures) on page C7-1652 */
  /*** Store multiple 4-element structures from four consecutive registers ST4
   * (multiple structures) on page C7-1969 */
  PUT7(Load_Store_Vector12, NM("ld4", "st4"),
       // No offset
       OPS(VREG8B_4D, PTR_O, XNSP, PTR_C), OPS(VREG16B_4D, PTR_O, XNSP, PTR_C),
       OPS(VREG4H_4D, PTR_O, XNSP, PTR_C), OPS(VREG8H_4D, PTR_O, XNSP, PTR_C),
       OPS(VREG2S_4D, PTR_O, XNSP, PTR_C), OPS(VREG4S_4D, PTR_O, XNSP, PTR_C),
       OPS(VREG2D_4D, PTR_O, XNSP, PTR_C));
  PUT7(Load_Store_Vector13, NM("ld4"),
       // Post-index
       OPS(VREG8B_4D, SPECIFIC6), OPS(VREG16B_4D, SPECIFIC8),
       OPS(VREG4H_4D, SPECIFIC6), OPS(VREG8H_4D, SPECIFIC8),
       OPS(VREG2S_4D, SPECIFIC6), OPS(VREG4S_4D, SPECIFIC8),
       OPS(VREG2D_4D, SPECIFIC8));
  /*** LD1R Load single 1-element structure and replicate to all lanes of one
   * register LD1R on page C7-1629 */
  PUT8(Load_Store_Vector14, NM("ld1r"),
       // No offset
       OPS(VREG8B_1D, PTR_O, XNSP2, PTR_C),
       OPS(VREG16B_1D, PTR_O, XNSP2, PTR_C),
       OPS(VREG4H_1D, PTR_O, XNSP2, PTR_C), OPS(VREG8H_1D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2S_1D, PTR_O, XNSP2, PTR_C), OPS(VREG4S_1D, PTR_O, XNSP2, PTR_C),
       OPS(VREG1D_1D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2D_1D, PTR_O, XNSP2, PTR_C));
  PUT1(Load_Store_Vector15, NM("ld1r"),
       // Post-index
  ...
  /*** LD2R Load single 2-element structure and replicate to all lanes of two
   * registers LD2R on page C7-1639 */
  PUT8(Load_Store_Vector16, NM("ld2r"),
       // No offset
       OPS(VREG8B_2D, PTR_O, XNSP2, PTR_C),
       OPS(VREG16B_2D, PTR_O, XNSP2, PTR_C),
       OPS(VREG4H_2D, PTR_O, XNSP2, PTR_C), OPS(VREG8H_2D, PTR_O, XNSP2, PTR_C),
       ...
       OPS(VREG2D_2D, PTR_O, XNSP2, PTR_C));
  PUT8(Load_Store_Vector17, NM("ld2r"),
       // Post-index
       OPS(VREG8B_2D, SPECIFIC1), OPS(VREG16B_2D, SPECIFIC1),
       OPS(VREG4H_2D, SPECIFIC2), OPS(VREG8H_2D, SPECIFIC2),
       OPS(VREG2S_2D, SPECIFIC3), OPS(VREG4S_2D, SPECIFIC3),
       OPS(VREG1D_2D, SPECIFIC4), OPS(VREG2D_2D, SPECIFIC4));
  /*** LD3R Load single 3-element structure and replicate to all lanes of three
   * registers LD3R on page C7-1649 */
  PUT8(Load_Store_Vector18, NM("ld3r"),
       // No offset
       OPS(VREG8B_3D, PTR_O, XNSP2, PTR_C),
       OPS(VREG16B_3D, PTR_O, XNSP2, PTR_C),
       OPS(VREG4H_3D, PTR_O, XNSP2, PTR_C), OPS(VREG8H_3D, PTR_O, XNSP2, PTR_C),
       ...
       OPS(VREG2D_3D, PTR_O, XNSP2, PTR_C));
  PUT8(Load_Store_Vector19, NM("ld3r"),
       // Post-index
       OPS(VREG8B_3D, SPECIFIC9), OPS(VREG16B_3D, SPECIFIC9),
       OPS(VREG4H_3D, SPECIFIC10), OPS(VREG8H_3D, SPECIFIC10),
       OPS(VREG2S_3D, SPECIFIC11), OPS(VREG4S_3D, SPECIFIC11),
       OPS(VREG1D_3D, SPECIFIC5), OPS(VREG2D_3D, SPECIFIC5));
  /*** LD4R Load single 4-element structure and replicate to all lanes of four
   * registers LD4R on page C7-1659 */
  PUT8(Load_Store_Vector20, NM("ld4r"),
       // No offset
       OPS(VREG8B_4D, PTR_O, XNSP2, PTR_C),
       OPS(VREG16B_4D, PTR_O, XNSP2, PTR_C),
       OPS(VREG4H_4D, PTR_O, XNSP2, PTR_C), OPS(VREG8H_4D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2S_4D, PTR_O, XNSP2, PTR_C), OPS(VREG4S_4D, PTR_O, XNSP2, PTR_C),
       OPS(VREG1D_4D, PTR_O, XNSP2, PTR_C),
       OPS(VREG2D_4D, PTR_O, XNSP2, PTR_C));
  PUT8(Load_Store_Vector21, NM("ld4r"),
       // Post-index
       OPS(VREG8B_4D, SPECIFIC2), OPS(VREG16B_4D, SPECIFIC2),
       OPS(VREG4H_4D, SPECIFIC3), OPS(VREG8H_4D, SPECIFIC3),
       OPS(VREG2S_4D, SPECIFIC4), OPS(VREG4S_4D, SPECIFIC4),
       OPS(VREG1D_4D, SPECIFIC6), OPS(VREG2D_4D, SPECIFIC6));
  ...
  PUT2(PrefetchMemory2, NM("prfm"), OPS(PRFOP, PTR_O, XNSP, PTR_C),
       OPS(PRFOP, PTR_O, XNSP, IMM12BIT_MUL8, PTR_C));
  /*** Prefetch memory (PC-relative offset) PRFM (literal) on page C6-1044 */
  PUT1(PrefetchMemory3, NM("prfm"), OPS(PRFOP, IMM19BIT_MUL4));
  /*** PRFUM Prefetch memory (unscaled offset) PRFUM on page C6-1048 */
  PUT2(PrefetchMemory4, NM("prfum"), OPS(PRFOP, PTR_O, XNSP, PTR_C),
  ...
  void putLoadStoreScalarSIMDAndFfloatingPoint() {
...

    {
      ...
      setTvAddressing(tv_SPECIFIC64_2, jtv_SPECIFIC64_2, tmpX, tv_IMM9BIT_PM,
                      TP_PreImm);
      setTvAddressing(tv_SPECIFIC64_3, jtv_SPECIFIC64_3, tmpX, tv_IMM7BIT_MUL4,
                      TP_PostImm);
      setTvAddressing(tv_SPECIFIC12, jtv_SPECIFIC12, tmpX, tv_IMM7BIT_MUL8,
                      TP_PostImm);
      setTvAddressing(tv_SPECIFIC13, jtv_SPECIFIC13, tmpX, tv_IMM7BIT_MUL16,
                      TP_PostImm);

      setTvAddressing(tv_SPECIFIC14, jtv_SPECIFIC14, tmpX, tv_IMM7BIT_MUL4,
                      TP_PreImm);
      setTvAddressing(tv_SPECIFIC15, jtv_SPECIFIC15, tmpX, tv_IMM7BIT_MUL8,
                      TP_PreImm);
      setTvAddressing(tv_SPECIFIC16, jtv_SPECIFIC16, tmpX, tv_IMM7BIT_MUL16,
                      TP_PreImm);

      setTvAddressing(tv_SPECIFIC0, jtv_SPECIFIC0, tv_XNSP2, tv_IMM7BIT_PM_MUL4,
                      TP_PostImm);
      setTvAddressing(tv_SPECIFIC1, jtv_SPECIFIC1, tv_XNSP2, tv_IMM7BIT_PM_MUL8,
                      TP_PostImm);
      setTvAddressing(tv_SPECIFIC2, jtv_SPECIFIC2, tv_XNSP2,
                      tv_IMM7BIT_PM_MUL16, TP_PostImm);

      setTvAddressing(tv_SPECIFIC3, jtv_SPECIFIC3, tv_XNSP2, tv_IMM9BIT_PM,
                      TP_PostImm);
    }
  ...
  void putLoadStoreVector() {
    ...
    setTvAddressing(tv_SPECIFIC4, jtv_SPECIFIC4, tv_XNSP2, tmpImm16,
                    TP_PostImm);
    setTvAddressing(tv_SPECIFIC5, jtv_SPECIFIC5, tv_XNSP2, tmpImm24,
                    TP_PostImm);
    setTvAddressing(tv_SPECIFIC6, jtv_SPECIFIC6, tv_XNSP2, tmpImm32,
                    TP_PostImm);
    setTvAddressing(tv_SPECIFIC7, jtv_SPECIFIC7, tv_XNSP2, tmpImm48,
                    TP_PostImm);
    setTvAddressing(tv_SPECIFIC8, jtv_SPECIFIC8, tv_XNSP2, tmpImm64,
                    TP_PostImm);
    setTvAddressing(tv_SPECIFIC9, jtv_SPECIFIC9, tv_XNSP2, tmpImm3, TP_PostImm);
    setTvAddressing(tv_SPECIFIC10, jtv_SPECIFIC10, tv_XNSP2, tmpImm6,
                    TP_PostImm);
    setTvAddressing(tv_SPECIFIC11, jtv_SPECIFIC11, tv_XNSP2, tmpImm12,
                    TP_PostImm);
