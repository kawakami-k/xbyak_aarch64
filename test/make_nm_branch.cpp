  ...
  */
  ...
  void putConditional_branch0() {
    ...
    for (size_t i = 0; i < cond.size(); i++) {
      if (isXbyak_) {
        std::cout << "b(" << cond[i] << ", " << tv_IMM19BIT_MUL4[0] << ");"
                  << std::endl;
      } else {
  ...
  PUT2(Conditional_branch1, NM("cbnz"), OPS(WREG, IMM19BIT_MUL4),
       OPS(XREG, IMM19BIT_MUL4));
  /**** CBZ Compare and branch if zero ¡Þ1MB CBZ on page C6-753 */
  PUT2(Conditional_branch2, NM("cbz"), OPS(WREG, IMM19BIT_MUL4),
       OPS(XREG, IMM19BIT_MUL4));
  /**** TBNZ Test bit and branch if nonzero ¡Þ32KB TBNZ on page C6-1224 */
  PUT2(Conditional_branch3, NM("tbnz"), OPS(WNZR, IMM5BIT, IMM14BIT_MUL4),
       OPS(XNZR, IMM6BIT, IMM14BIT_MUL4));
  /**** TBZ Test bit and branch if zero ¡Þ32KB TBZ on page C6-1225 */
  PUT2(Conditional_branch4, NM("tbz"), OPS(WNZR, IMM5BIT, IMM14BIT_MUL4),
       OPS(XNZR, IMM6BIT, IMM14BIT_MUL4));
  ...
  PUT1(Undinditional_branch0, NM("b"), OPS(IMM26BIT_MUL4));
  /**** BL Branch with link ¡Þ128MB BL on page C6-735 */
  PUT1(Undinditional_branch1, NM("bl"), OPS(IMM26BIT_MUL4));
  ...
  PUT1(Unconditional_branch_register0, NM("blr", "br"), OPS(XREG));
  /**** RET Return from subroutine RET on page C6-1053 */
  PUT1(Unconditional_branch_register1, NM("ret"), OPS(XREG));
   ...
   */
  PUT1(Exception_generation_and_return0, NM("brk", "hlt", "hvc", "smc", "svc"),
       OPS(IMM16BIT));
  /**** Exception return */
  /***** ERET Exception return using current ELR and SPSR ERET on page C6-816 */
  PUT0(Exception_generation_and_return1, NM("eret"));
  /**** Debug state */
  ...
  PUT1(Exception_generation_and_return2, NM("dcps1", "dcps2", "dcps3"),
       OPS(IMM16BIT));
  /***** DRPS Debug restore PE state DRPS on page C6-807 */
  PUT0(Exception_generation_and_return3, NM("drps"));
  ...
  /**** MSR Move general-purpose register to System register MSR (register) on
   * page C6-1014 */
  ...
  /**** IC Instruction cache maintenance IC on page C6-825 and Table C5-1 on
   * page C5-347 */
  /***** Xbyak for AArch64 does not suppport IC instruction. */
  /**** DC Data cache maintenance DC on page C6-801 and Table C5-1 on page
   * C5-347 */
  ...
  PUT0(Hint_instructions0, NM("nop", "yield", "wfe", "wfi", "sev", "sevl"));
  /**** HINT Unallocated hint HINT on page C6-821 */
  PUT1(Hint_instructions1, NM("hint"), OPS(IMM7BIT));
  ...
  PUT1(Barriers_and_CLREX_instructions0, NM("clrex"), OPS(IMM4BIT));
  /**** DSB Data synchronization barrier DSB on page C6-808 */
  /**** DMB Data memory barrier DMB on page C6-805 */
  PUT1(Barriers_and_CLREX_instructions1, NM("dsb", "dmb"), OPS(BARRIER));
  /**** CSDB Consumption of Speculative Data Barrier Consumption of Speculative
   * Data Barrier (CSDB) on page B2-105 */
  /**** ESB Error synchronization barrier ESB on page C6-818 */
  PUT0(Barriers_and_CLREX_instructions2, NM("csdb", "esb"));
  /**** TSB CSYNC Trace Synchronization Barrier Trace Synchronization Barrier
   * (TSB CSYNC) on page B2-106 */
  ...
  /**** PSSBB Physical Speculative Store Bypass Barrier Physical Speculative
   * Store Bypass Barrier (PSSBB) on page B2-105 */
  /**** SSBB Speculative Store Bypass Barrier Speculative Store Bypass Barrier
   * (SSBB) on page B2-105 */
  PUT0(Barriers_and_CLREX_instructions6, NM("pssbb", "ssbb"));
  ...
  void putBarriers_and_CLREX_instructions_core() {
    ...
    //    putBarriers_and_CLREX_instructions3(); // ARMv8.4 instruction. Doese
    //    A64FX support this instruction?
    putBarriers_and_CLREX_instructions4();
    putBarriers_and_CLREX_instructions5();
    // putBarriers_and_CLREX_instructions6(); // Does armv8.4-a support pssbb
    // and ssbb instructions?
  }

  /*** Pointer authentication instructions on page C3-174. */
  /**** Table C3-11 Instructions that add a PAC */
  /***** PACIASP Add PAC to instruction address using APIAKey_EL1 and SP PACIA,
   * PACIA1716, PACIASP, PACIAZ, PACIZA on page C6-1038 */
   ...
   */
  /***** PACIA Add PAC to instruction address using APIAKey_EL1, registers
   * PACIA, PACIA1716, PACIASP, PACIAZ, PACIZA on page C6-1038 */
  /***** PACIA, PACIA1716, PACIASP, PACIAZ, PACIZA on page C6-1038 */
  PUT1(Pointer_authentication_instructions0, NM("pacia"), OPS(XREG, XNSP));
  PUT1(Pointer_authentication_instructions1, NM("paciza"), OPS(XREG));
  PUT0(Pointer_authentication_instructions2,
       NM("pacia1716", "paciasp", "paciaz"));
  /***** PACIBSP Add PAC to instruction address using APIBKey_EL1 and SP PACIB,
   * PACIB1716, PACIBSP, PACIBZ, PACIZB on page C6-1040 */
  /***** PACIB Add PAC to instruction address using APIBKey_EL1, registers
   ...
   */
  /***** PACIB, PACIB1716, PACIBSP, PACIBZ, PACIZB on page C6-1040 */
  PUT1(Pointer_authentication_instructions3, NM("pacib"), OPS(XREG, XNSP));
  PUT1(Pointer_authentication_instructions4, NM("pacizb"), OPS(XREG));
  PUT0(Pointer_authentication_instructions5,
       NM("pacib1716", "pacibsp", "pacibz"));
  /***** PACDA Add PAC to data address using APDAKey_EL1, registers PACDA,
   * PACDZA on page C6-1035 */
  /***** PACDB Add PAC to data address using APDBKey_EL1, registers PACDB,
   * PACDZB on page C6-1036 */
  PUT1(Pointer_authentication_instructions6, NM("pacda", "pacdb"),
       OPS(XREG, XNSP));
  PUT1(Pointer_authentication_instructions7, NM("pacdza", "pacdzb"), OPS(XREG));
  /***** PACGA Add generic PAC using APGAKey_EL1, registers PACGA on page
   * C6-1037 */
  PUT1(Pointer_authentication_instructions8, NM("pacga"),
       OPS(XREG, XREG, XNSP));

  /**** Table C3-12 Instructions that authenticate a PAC */
  /***** AUTIA Authenticate PAC for instruction address using APIAKey_EL1,
   * registers AUTIA, AUTIA1716, AUTIASP, AUTIAZ, AUTIZA on page C6-717 */
  /***** AUTIB Authenticate PAC for instruction address using APIBKey_EL1,
   * registers AUTIB, AUTIB1716, AUTIBSP, AUTIBZ, AUTIZB on page C6-719 */
  /***** AUTDA Authenticate PAC for data address using APDAKey_EL1, registers
   * AUTDA, AUTDZA on page C6-715 */
  /***** AUTDB Authenticate PAC for data address using APDBKey_EL1, registers
   * AUTDB, AUTDZB on page C6-716 */
  PUT1(Pointer_authentication_instructions9,
       NM("autia", "autib", "autda", "autdb"), OPS(XREG, XNSP));
  /***** AUTIZA Authenticate PAC for instruction address using APIAKey_EL1,
   * register and zero AUTIA, AUTIA1716, AUTIASP, AUTIAZ, AUTIZA on page C6-717
   */
  /***** AUTIZB Authenticate PAC for instruction address using APIBKey_EL1,
   * register and zero AUTIB, AUTIB1716, AUTIBSP, AUTIBZ, AUTIZB on page C6-719
   */
  /***** AUTDZA Authenticate PAC for data address using APDAKey_EL1, register
   * and zero AUTDA, AUTDZA on page C6-715 */
  /***** AUTDZB Authenticate PAC for data address using APDBKey_EL1, register
   * and zero AUTDB, AUTDZB   */
  PUT1(Pointer_authentication_instructions10,
       NM("autiza", "autizb", "autdza", "autdzb"), OPS(XREG));
  /***** AUTIA1716 Authenticate PAC for instruction address X17 using
   * APIAKey_EL1 and X16 */
  /***** AUTIASP Authenticate PAC for instruction address using APIAKey_EL1 and
   * SP AUTIA, AUTIA1716, AUTIASP, AUTIAZ, AUTIZA on */
  /***** AUTIAZ Authenticate PAC for instruction address using APIAKey_EL1 and
   * zero page C6-717 */
  /***** AUTIB1716 Authenticate PAC for instruction address X17 using
   * APIBKey_EL1 and X16 */
  /***** AUTIBZ Authenticate PAC for instruction address using APIBKey_EL1 and
   * zero page C6-719 */
  /***** AUTIBSP Authenticate PAC for instruction address using APIBKey_EL1 and
   * SP AUTIB, AUTIB1716, AUTIBSP, AUTIBZ, AUTIZB on */
  ...
  PUT1(Pointer_authentication_instructions12, NM("xpacd", "xpaci"), OPS(XREG));
  /***** XPACLRI Strip instruction address PAC from LR XPACD, XPACI, XPACLRI on
   * page C6-1252 */
  PUT0(Pointer_authentication_instructions13, NM("xpaclri"));

  /**** Table C3-14 Combined pointer authentication instructions */
  /***** RETAA Authenticate PAC for LR using APIAKey_EL1 and SP, and return
   * RETAA, RETAB on page C6-1054 */
  /***** RETAB Authenticate PAC for LR using APIBKey_EL1 and SP, and return */
  /***** ERETAA Authenticate PAC for ELR using APIAKey_EL1 and SP, and exception
   * return ERETAA, ERETAB on page C6-817 */
  /***** ERETAB Authenticate PAC for ELR using APIBKey_EL1 and SP, and exception
   * return */
  PUT0(Pointer_authentication_instructions14,
       NM("retaa", "retab", "eretaa", "eretab"));
  /***** BRAAZ Authenticate PAC using APIAKey_EL1 (register and zero), and
   * branch BRAA, BRAAZ, BRAB, BRABZ on page C6-740 */
  /***** BRABZ Authenticate PAC using APIBKey_EL1 (register and zero), and
   * branch */
  /***** BLRAAZ Authenticate PAC using APIAKey_EL1 (register and zero), and
   * branch with link BLRAA, BLRAAZ, BLRAB, BLRABZ on page C6-737 */
  /***** BLRABZ Authenticate PAC using APIBKey_EL1 (register and zero), and
   * branch with link */
  PUT1(Pointer_authentication_instructions15,
       NM("braaz", "brabz", "blraaz", "blrabz"), OPS(XREG));
  /***** BRAA Authenticate PAC using APIAKey_EL1 (registers), and branch BRAA,
   * BRAAZ, BRAB, BRABZ on page C6-740 */
  /***** BRAB Authenticate PAC using APIBKey_EL1 (registers), and branch */
  /***** BLRAA Authenticate PAC using APIAKey_EL1 (registers), and branch with
   * link BLRAA, BLRAAZ, BLRAB, BLRABZ on page C6-737 */
  /***** BLRAB Authenticate PAC using APIBKey_EL1 (registers), and branch with
   * link */
  PUT1(Pointer_authentication_instructions16,
       NM("braa", "brab", "blraa", "blrab"), OPS(XREG, XNSP));
  /***** LDRAA Authenticate PAC for data address using APDAKey_EL1 (register and
   * zero) and Load LDRAA, LDRAB on page C6-893 */
  /***** LDRAB Authenticate PAC for data address using APDBKey_EL1 (register and
   * zero) and Load */
  PUT2(Pointer_authentication_instructions17, NM("ldraa", "ldrab"),
       OPS(XREG, PTR_O, XNSP3, PTR_C),
       OPS(XREG, PTR_O, XNSP3, IMM10BIT_PM_MUL8, PTR_C));
  PUT1(Pointer_authentication_instructions18, NM("ldraa", "ldrab"),
       // OPS(XREG, PRE_PTR_O, XNSP, PRE_PTR_C), // Gnu as v2.3.0 can't encode
       // ldraa and ldrab instructions without offset.
       OPS(XREG, SPECIFIC0));
  ...
  void putConditional_branch() { putConditional_branch_core(); }
  ...
  void putHint_instructions() { putHint_instructions_core(); }
  ...
  void putPointer_authentication_instructions() {
    ...
    setTvAddressing(tv_SPECIFIC0, jtv_SPECIFIC0, tv_XNSP3, tv_IMM10BIT_PM_MUL8,
                    TP_PreImm);
